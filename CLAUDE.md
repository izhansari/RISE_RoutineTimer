...
RISE_RoutineTimer/
  ...
  RoutineListView.swift         – step list editor, presented as a sheet from the Run tab's pencil
...
87 tests — mostly pure value-type math against synthetic dates; the suite runs in well under a second. The build should be warning-free; keep it that way.
...
- **Auto-next is shown twice, both times without status wording.** A small bordered `AUTO` / `MANUAL` capsule sits under the step name, and the checkmark's border echoes it — **dashed** for auto, **solid** for manual. The button is one quiet ghosted size (`checkDiameter`) for every step; the reserved slot is `buttonSlotHeight`, so the type above never shifts.
...
- `RoutineStep.autoShowNotes` (default on) opens the sheet as a step begins, and is toggled by a plainly labelled "Open automatically" switch at the foot of the sheet. An icon-only version in the toolbar was tried and dropped: compact, but it gave no clue what it controlled.
...
- `RunSheetView` draws each section as a titled hairline box (`section(_:content:)`), so where one group ends and the next begins is drawn rather than implied by whitespace — evenly spaced rows made it unclear which numbers belonged together. Rows are 8pt tall; sections sit 16pt apart.
...
- **The Run tab's idle list is directly editable.** Tapping a step opens `StepEditorView` in a sheet; the `+` on each connector (`InsertConnector`) inserts a new step at exactly that point and opens it; an `ADD STEP` row appends. The pencil still opens the full list for reorder / delete / restore. Only while `engine.run == nil` — a finished run shows the frozen order, which may differ from the saved one after a move-to-end.
...
- Tabs are **Today / Run / History / Settings**. There is no Routine tab: the step list is edited from the pencil in the Run tab's top-left, next to the list you are already looking at — editing a routine from a different tab than the one displaying it was a needless round trip.
